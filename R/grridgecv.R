grridge.cv <-
function(grr,highdimdata,response,outerfold=length(response),fixedfolds=TRUE,recalibrate=FALSE){
#highdimdata<-datcenFarkas;response<-respFarkas;grr <- grFarkas;outerfold=10;fixedfolds<-T
#highdimdata<-datcenVerlaat;response<-respVerlaat;grr <- grVerlaat;outerfold=length(response);fixedfolds<-T;recalibrate=FALSE
...
#highdimdata<-mirnormcenTS_PFS;response<-survPFS;grr<-grMaartenSurv;outerfold=3;fixedfolds<-T;
  #grr=grNB2;highdimdata<-seqTrcen;response<-clinOSTr;outerfold=3;fixedfolds=TRUE;recalibrate=TRUE
model <- grr$model

#if(model=="survival") allobstimes <- sort(unique(response[response[,2]==1 ,1]))
...
comparelasso = arg$comparelasso, optllasso=arg$optllasso, compareEN = arg$compareEN, compareunpenal = arg$compareunpenal,modus=arg$modus)
...
if(model=="logistic" | model =="linear"){
    ...
    if(arg$compareEN) npreds <- npreds-1
    if(npreds>0){
        for(ell in 1:npreds){
        #ell<-1
        predobj <- penobj[[ell]]
  ...
  if(arg$selection){
      ...
      if(recalibrate){
        ...
        if(model=="logistic") refitmod <- glm(responseout ~  1 + datlp, family="binomial") else
          refitmod <- glm(responseout ~  1 + datlp, family="gaussian")
        print(paste("Recalibration intercept and slope for model selected variables:",round(refitmod$coefficients[1],3),";",
                    round(refitmod$coefficients[2],3)))
        predell2 <- predict(refitmod,type="response")
        predellall2 <- cbind(predellall2, predell2)
      }
  ...
  #NEW
  if(arg$comparelasso){ #lasso should be applied on unweighted matrix Xsam
  ...
  if(arg$compareunpenal){
      nadd <- arg$selection + arg$comparelasso + arg$compareEN
      take <- npreds+nadd + 1
      predobj <- penobj[[take]]
