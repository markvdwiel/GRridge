...
{\tt GRridge} package was based on these following publications:
...
Putri W. Novianti, Barbara C. Snoek, Saskia Wilting, Mark van de Wiel. (2017). Better diagnostic signatures from RNAseq data through use of auxiliary co-data. Bioinformatics. 33, 1572-1574. \citep{novianti2017}
...
%The {\tt GRridge} package is freely available online. We show {\tt GRridge} installation from Bioconductor
...
The GRridge package depends on these following R packages: {\tt penalized} \citep{goeman2010l1}, {\tt glmnet} \citep{glmnet1} ,{\tt survival} (Therneau, 2015) and {\tt Iso} (Turner, 2015).
...
\item coDataWurdinger		: A list containing co-data (i) sets from immunologic signature pathway and (ii)platelets expressed genes
\end{itemize}
...
@
\item co-data 2: a partition based on a list of platelets expressed genes. 
...
@
\item co-data 3: a partition based on chromosomal location. A list of chromosomal location based on {\tt biomaRt} data bases.
<<echo=TRUE,print=FALSE, results=hide>>=
